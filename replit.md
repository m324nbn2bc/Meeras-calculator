...
- **Saved calculations history** (`lib/history.ts`, `app/history.tsx`): AsyncStorage-backed save/load/delete (`meeras.history.v1`, max 50). "Save" bookmark button on every result screen; "Saved Calculations" tertiary button on home screen. Each entry stores label (heir names), estate, madhab, currency, serialised WizardState. Tapping a history entry reopens the result screen.
- **PDF / Share export**: "Share / Export PDF" button on result screen. Builds a styled HTML document from the current result, calls `expo-print` (`printToFileAsync` on native, `printAsync` on web) and `expo-sharing` (`shareAsync`) to hand off the PDF.
